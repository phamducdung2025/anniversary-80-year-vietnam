...
const Placeholder = () => (
    <div className="flex flex-col items-center justify-center h-full text-neutral-500 group-hover:text-neutral-300 transition-colors duration-300">
        ...
        <span className="font-sriracha text-xl">Tải ảnh lên</span>
    </div>
);
...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, dragConstraintsRef, onShake, onDownload, onClick, isMobile }) => {
    ...
    const isClickable = status === 'done' && imageUrl && onClick;
    ...
    const cardInnerContent = (
        <>
            ...
            <div className="absolute bottom-4 left-4 right-4 text-center px-2">
                <p className={cn(
                    "font-sriracha text-lg truncate",
    ...
    if (isMobile) {
        return (
            <div 
                className={cn(
                    "bg-neutral-100 dark:bg-neutral-100 !p-4 !pb-16 flex flex-col items-center justify-start aspect-[3/4] w-80 max-w-full rounded-md shadow-lg relative",
