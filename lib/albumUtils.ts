...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.font = `bold 100px 'Lobster', cursive`;
    ctx.fillText('Kỷ niệm 80 năm Quốc Khánh Việt Nam', canvasWidth / 2, 150);

    ctx.font = `50px 'Roboto', sans-serif`;
    ctx.fillStyle = '#555';
    ctx.fillText('2/9/1945 - 2/9/2025', canvasWidth / 2, 220);
    ...
    const reversedImages = [...imagesWithThemes].reverse();
    reversedImages.forEach(({ theme, img }, reversedIndex) => {
        // Calculate the original index to determine grid position
        const index = imagesWithThemes.length - 1 - reversedIndex;
        ...
        ctx.fillStyle = '#222';
        ctx.font = `60px 'Sriracha', cursive`;
        ctx.textAlign = 'center';
